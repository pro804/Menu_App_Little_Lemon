...
// Update CATEGORY_MAP to use plural keys
  const CATEGORY_MAP = {
...
const fetchData = async () => {
  ...
  try {
    ...
    const transformedData = data.menu.map((item) => {
      const rawCategory = item.category?.title?.toLowerCase();
    ...
    (async () => {
      try {
        ...
        if (menuItems === undefined || !menuItems.length) {
           menuItems = await fetchData(); // update the outer scope variable
  ...
  const sectionIndex = sections.indexOf(item.title);
  if (filterSelections.every((item) => item === false)) {
    return true;
  }
  return filterSelections[sectionIndex];
});
